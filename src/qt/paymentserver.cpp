...
const int HASHCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString HASHCOIN_IPC_PREFIX("hashcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(HASHCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(HASHCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(HASHCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click hashcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start hashcoin: click-to-pay handler");
    else
...
{
    // clicking on hashcoin: URLs creates FileOpen events on the Mac:
